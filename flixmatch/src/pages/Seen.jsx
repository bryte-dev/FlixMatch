...
function Seen() {
  ...
  useEffect(() => {
    const fetchSeenMovies = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_BACK_API_URL}/seen`, { withCredentials: true });
        setMovies(res.data);
      } catch (error) {
  ...
  const submitReview = async (movieId) => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_BACK_API_URL}/reviews/${movieId}`,
  ...
  const restoreToWatchlist = async (movieId) => {
    try {
      await axios.put(`${import.meta.env.VITE_BACK_API_URL}/seen/${movieId}/remove`, {}, { withCredentials: true });

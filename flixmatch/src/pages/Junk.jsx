...
function Junk() {
  ...
  useEffect(() => {
    const fetchJunk = async () => {
      try {
        const res = await axios.get(`${process.env.VITE_API_URL}/junk`, { withCredentials: true });
  ...
  const restoreFromJunk = async (movieId) => {
    try {
      const response = await axios.put(`${process.env.VITE_API_URL}/junk/${movieId}/restore`);

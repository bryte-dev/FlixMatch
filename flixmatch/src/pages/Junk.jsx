...
function Junk() {
  ...
  useEffect(() => {
    const fetchJunk = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_BACK_API_URL}/junk`, { withCredentials: true });
  ...
  const restoreFromJunk = async (movieId) => {
    try {
      const response = await axios.put(`${import.meta.env.VITE_BACK_API_URL}/junk/${movieId}/restore`);
